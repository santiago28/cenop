      ...
      <div class="panel-body">
        ...
        <div class="table-responsive col-md-10" style="border-radius: 5px;width: 100%;margin: 0px auto; float: none;">
          <table class="table table-bordered" id="mytable">
            ...
            <tbody>
              {% if ordenproduccion is defined %}
              {% for orden in ordenproduccion %}
              <tr>
                ...
                <td>
                  {% if tipoUsuario == 2 %}
                  {{ select("cambiarCliente", listaempresas, "value" : orden.idEmpresa, "class" : "form-control cambiarCliente estadoCliente") }}
                  {% else %}
                  {% if tipoTecnico == 2 %}
                  {{ select("cambiarCliente", listaempresas, "value" : orden.idEmpresa, "class" : "form-control cambiarCliente estadoCliente") }}
                  {% else %}
                  {{ select("cambiarCliente", listaempresas, "value" : orden.idEmpresa, "class" : "form-control cambiarCliente") }}
                  {% endif %}
                  {% endif %}
                </td>
                ...
                <td>
                  {% if tipoUsuario == 2 %}
                  {{ select("estadoPaciente", estadoPaciente, "value" : orden.estadoPaciente, "class" : "form-control estadoPaciente estadoCliente") }}
                  {% else %}
                  {% if tipoTecnico == 2 %}
                  {{ select("estadoPaciente", estadoPaciente, "value" : orden.estadoPaciente, "class" : "form-control estadoPaciente estadoCliente") }}
                  {% else %}
                  {{ select("estadoPaciente", estadoPaciente, "value" : orden.estadoPaciente, "class" : "form-control estadoPaciente") }}
                  {% endif %}
                  {% endif %}
                </td>
                {% if tipoUsuario != 2 %}
                <td>{{ link_to("ordenproduccion/edit/"~orden.idOrden, "&#xE8B6;", 'class':'material-icons', 'style': 'color:#757575; text-decoration: none;') }}</td>
                {# <td>{{ link_to("ordenproduccion/delete/"~orden.idOrden, "&#xE872;", 'class':'material-icons', 'style': 'color:#757575; text-decoration: none;') }}</td> #}
                <td><a href="#eliminar_elemento" rel="tooltip" title="Eliminar" class="material-icons eliminar_fila" style="color:#757575; text-decoration: none;" data-toggle = "modal" id="{{ url("ordenproduccion/delete/"~orden.idOrden) }}">&#xE872;</a></td>
                {% endif %}
                {% if tipoUsuario != 1 %}
      ...
      <div class="panel-footer">
        <div class="form-group" style="overflow:hidden;text-align:right;">
          <div class="col-sm-offset-2 col-sm-10">
            {# {{ link_to("Ordenproduccion/new", "Registrar Órden", "class": "btn btn-primary") }} #}
            {% if tipoUsuario != 2 %}
...
<div id="ModalOrden" class="modal fade" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      {{ form("Ordenproduccion/create", "method":"post", "autocomplete" : "off") }}
